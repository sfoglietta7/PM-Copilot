...
Key principle from Lenny: "The best PRDs are problem-oriented. They define the problem so clearly that an engineer could propose a better solution than the PM did." â€” Lenny's PRD guide

## Step 1 â€” Load Context

Read in parallel:
- `memory/user-profile.md` â€” PRD format preference (brief / detailed / bullet-heavy / narrative), product context, stakeholders
- `context/product/personas.md` (fallback: template)
- `context/product/roadmap.md` (fallback: template) â€” for OKR linkage and strategic fit
- `context/company/analytics-baseline.md` â€” for realistic success metrics
- `context/company/past-prds.md` â€” if it exists, match the team's tone and level of detail
- `context/templates/prd-template.md` â€” the output template

Apply `prd_format_preference` from memory. If "brief", keep sections tight. If "detailed", expand with examples.

## Step 2 â€” Parse the Input
...
## Step 3 â€” Clarify If Needed (One Question Only)
...
**Solution:** Describe the core mechanic. Include explicit out-of-scope items â€” minimum 2. These prevent scope creep and make alignment faster.

**Key Flows:** Write 2â€“3 flows covering: (1) happy path, (2) an edge case or alternative path. Each flow should be numbered steps, not prose.
...
**Acceptance Criteria:** 4â€“6 binary criteria. Each must be testable in isolation by QA. Avoid: "the feature works correctly" (not testable). Use: "When a user [action], the system [specific response] within [time/condition]."
...
**Success Metrics:** 2â€“3 metrics with baseline, target, timeframe, and measurement method. All must be outcome-oriented and measurable within 30 days of ship.
